...
function getSessionStamp() {
  if (previousSessionStamps.length > 0 && Math.random() < 0.01) {
    return previousSessionStamps[
      Math.floor(Math.random() * previousSessionStamps.length)
    ];
  }
    ...
    for (let i = 0; i < hitsCount; i++) {
      ...
      const params = {
        headers: {
          ...
          "user-agent":
            "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) QtWebEngine/6.8.3 Chrome/122.0.0.0 Safari/537.36",
        },
        tags: {
