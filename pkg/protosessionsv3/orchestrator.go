...
func (o *Orchestrator) processBatch(
	...
	// changing the ownership of the hit. This orchestrator uses strategy pattern to allow for different
	...
	// session is on the same partition or not).
	//
	// This part of code is only relevant when `EvictWholeProtosessionStrategy` is used. It may happen, that
	// a hit evicted in the past from another partition comes here and the session it should be attributed to
	// is already closed. groupHitsByBucket handles bucket locking and filtering of expired/in-progress buckets.
	newBatch, hitsToDrop, err := o.groupHitsByBucket(hitsBatch, batchSettingsRegistry)
	if err != nil {
	...
	// Apply eviction strategy to conflicting hits. Depending on the strategy, hits are either
	// rewritten in-place or their entire proto-sessions are marked for eviction and re-queuing.
	...
	// Build requests to save hits and mark proto-sessions for closing at the appropriate bucket.
	...
	// Cleanup phase: remove evicted proto-session data and metadata for dropped hits.
...
// groupHitsByBucket partitions hits into timing wheel buckets, locks each bucket, and filters out
// hits destined for buckets that are currently being processed or already expired.
func (o *Orchestrator) groupHitsByBucket(
	hitsBatch []*hits.Hit,
