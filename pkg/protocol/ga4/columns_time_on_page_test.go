...
func simpleHitsWithTime(h [][4]any) columntests.TestHits {
	...
	for i, hit := range h {
		...
		if hit[1] != nil {
			...
			allHits[i].QueryParams.Add("dl", pageLocation)
		}
		if hit[2] != nil {
			...
			allHits[i].QueryParams.Add("dt", pageTitle)
		}
		offsetSeconds, ok := hit[3].(int)
		if !ok {
			panic("time offset must be int")
		}
		allHits[i].ServerReceivedTime = baseTime.Add(time.Duration(offsetSeconds) * time.Second)
	}
	return allHits
}
