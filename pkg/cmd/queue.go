...
func buildQueue(ctx context.Context, cmd *cli.Command) (*Queue, error) {
	...
	case "objectstorage":
		...
		q, err := buildObjectStorageQueue(ctx, bucket, cmd.String(queueObjectPrefixFlag.Name))
		if err != nil {
			_ = cleanup()
			return nil, err
		}
...
func buildObjectStorageQueue(ctx context.Context, bucket *blob.Bucket, prefix string) (*Queue, error) {
