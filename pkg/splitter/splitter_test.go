	...
	cases := []struct {
		...
		},
		{
			...
			session: func() *schema.Session {
				baseTime := time.Now()
				hit1 := hits.New()
				hit1.ServerReceivedTime = baseTime
				hit2 := hits.New()
				hit2.ServerReceivedTime = baseTime.Add(2 * time.Minute)
				hit3 := hits.New()
				hit3.ServerReceivedTime = baseTime.Add(4 * time.Minute)
		...
		},
		{
			...
			session: func() *schema.Session {
				...
				hit2.ServerReceivedTime = baseTime.Add(2 * time.Minute)
				hit3 := hits.New()
				hit3.ServerReceivedTime = baseTime.Add(5 * time.Minute)
				hit4 := hits.New()
				hit4.ServerReceivedTime = baseTime.Add(7 * time.Minute)
				return &schema.Session{
					Events: []*schema.Event{
						schema.NewEvent(hit1),
						schema.NewEvent(hit2),
		...
		},
		{
			...
			session: func() *schema.Session {
				...
				hit1.ServerReceivedTime = baseTime
				hit2 := hits.New()
				hit2.ServerReceivedTime = baseTime.Add(5 * time.Minute)
				hit3 := hits.New()
				hit3.ServerReceivedTime = baseTime.Add(10 * time.Minute)
				hit4 := hits.New()
				hit4.ServerReceivedTime = baseTime.Add(15 * time.Minute)
				return &schema.Session{
					Events: []*schema.Event{
						schema.NewEvent(hit1),
						schema.NewEvent(hit2),
		...
		},
		{
			...
			session: func() *schema.Session {
				...
				hit3.ServerReceivedTime = baseTime.Add(4 * time.Minute)
				hit4 := hits.New()
				hit4.ServerReceivedTime = baseTime.Add(6 * time.Minute)
				return &schema.Session{
					Events: []*schema.Event{
		...
		},
		{
			...
			session: func() *schema.Session {
				...
				hit1.ServerReceivedTime = baseTime
				hit2 := hits.New()
				hit2.ServerReceivedTime = baseTime.Add(1 * time.Minute)
				hit3 := hits.New()
				hit3.ServerReceivedTime = baseTime.Add(2 * time.Minute)
				hit4 := hits.New()
				hit4.ServerReceivedTime = baseTime.Add(3 * time.Minute)
				hit5 := hits.New()
				hit5.ServerReceivedTime = baseTime.Add(6 * time.Minute)
				hit6 := hits.New()
				hit6.ServerReceivedTime = baseTime.Add(7 * time.Minute)
				return &schema.Session{
					Events: []*schema.Event{
