...
test.describe('GA4 Duplicator', () => {
  ...
  async function verifyDuplication(page: any, triggerAction: () => Promise<void>, method: 'GET' | 'POST') {
    const ga4RequestPromise = page.waitForRequest((req: Request) => 
      req.url().includes(GA4_ORIGIN) && req.method() === method
    );
    const d8aRequestPromise = page.waitForRequest((req: Request) => 
      req.url().includes(D8A_ORIGIN) && req.method() === method
    );
