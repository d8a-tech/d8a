...
(window as any).createGA4Duplicator = function (options: GA4DuplicatorOptions) {
    ...
    class FetchInterceptor implements NetworkInterceptor {
        install(ctx: InterceptorContext): void {
            ...
            window.fetch = function (this: any, resource: RequestInfo | URL, config?: RequestInit): Promise<Response> {
                const requestUrl = typeof resource === "string" ? resource : resource instanceof URL ? resource.toString() : (resource as any).url;
                    ...
                    if (upperMethod === "GET") {
                        ...
                        prepareBodyPromise.then((dupBody) => {
                            originalFetch(duplicateUrl, { method: "POST", body: dupBody, keepalive: true }).catch((error) => {
                                if (ctx.debug) console.error("gtm interceptor: error duplicating POST fetch:", error);
                            });
                        });
                    }
                ...
                if (this._requestUrl && ctx.isTargetUrl(this._requestUrl)) {
                    ...
                    try {
                        ...
                        if (method === "GET") {
                            ...
                            fetch(duplicateUrl, { method: "POST", body: body as any, keepalive: true }).catch((error) => {
                                if (ctx.debug) console.error("gtm interceptor: error duplicating POST xhr:", error);
                            });
                        }
                    } catch (dupErr) {
    ...
    class BeaconInterceptor implements NetworkInterceptor {
        install(ctx: InterceptorContext): void {
            ...
            navigator.sendBeacon = function (this: any, url: string | URL, data?: BodyInit | null): boolean {
    ...
    class ScriptInterceptor implements NetworkInterceptor {
        install(ctx: InterceptorContext): void {
            try {
                ...
                HTMLScriptElement.prototype.setAttribute = function (this: any, name: string, value: string) {
                    try {
