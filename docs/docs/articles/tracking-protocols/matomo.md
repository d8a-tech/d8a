...
-   `search` --- The Site Search keyword used to track searches performed on your own site. This is not an external search engine referrer term (for example, not `referer_keyword`). When specified, the request will not be tracked as a normal pageview but will instead be tracked as a [Site Search](https://matomo.org/docs/site-search/) request.
