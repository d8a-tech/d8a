...
if (typeof window.Matomo !== "object") {
  window.Matomo = window.Piwik = (function () {
    ...
    function executePluginMethod(methodName, params, callback) {
      ...
      for (i in plugins) {
        if (Object.prototype.hasOwnProperty.call(plugins, i)) {
          isFunction =
            plugins[i] && "function" === typeof plugins[i][methodName];
    ...
    function addUrlParameter(url, name, value) {
      ...
      return (
        baseUrl + encodeWrapper(name) + "=" + encodeWrapper(value) + urlHash
      );
    }
    ...
    var content = {
      ...
      findMediaUrlInNode: function (node) {
        ...
        if (
          -1 !== indexOfArray(mediaElements, elementName) &&
          query.findFirstNodeHavingAttributeWithValue(node, "src")
        ) {
          var sourceNode = query.findFirstNodeHavingAttributeWithValue(
            node,
            "src",
          );
      ...
      buildInteractionRequestParams: function (
        interaction,
        name,
        piece,
        target,
      ...
      buildImpressionRequestParams: function (name, piece, target) {
        var params =
          "c_n=" + encodeWrapper(name) + "&c_p=" + encodeWrapper(piece);
    ...
    function Tracker(trackerUrl, siteId) {
      ...
      var /*<DEBUG>*/
        ...
        configCampaignKnownParameters = [
          ...
          "mtm_content",
          "pk_content",
          "utm_content", // campaign source
          "mtm_cid",
          "pk_cid",
          "utm_id",
          "mtm_clid", // campaign (click) id
          "mtm_group",
          "pk_group", // campaign group
          "mtm_placement",
          "pk_placement", // campaign placement
        ],
      ...
      function shouldIgnoreCampaignForReferrer(referrerUrl) {
        ...
        if (
          configIgnoreCampaignsForReferrers &&
          configIgnoreCampaignsForReferrers.length
        ) {
          for (i = 0; i < configIgnoreCampaignsForReferrers.length; i++) {
            ...
            if (
              isSameHost(currentReferrerHostName, aliasHost) &&
              isSitePath(currentReferrerPath, aliasPath)
            ) {
              return true;
            }
          }
      ...
      function resolveRelativeReference(baseUrl, url) {
        ...
        if (url.slice(0, 1) === "/") {
          return (
            getProtocolScheme(baseUrl) + "://" + getHostName(baseUrl) + url
          );
        }
      ...
      function injectBrowserFeaturesAndClientHints(request) {
        ...
        if (clientHints) {
          appendix =
            "&uadata=" + encodeWrapper(windowAlias.JSON.stringify(clientHints));
        }
      ...
      function getRequest(request, customData, pluginMethod) {
        ...
        if (customData) {
          request +=
            "&data=" + encodeWrapper(windowAlias.JSON.stringify(customData));
        } else if (configCustomData) {
          request +=
            "&data=" +
            encodeWrapper(windowAlias.JSON.stringify(configCustomData));
        }
        ...
        function appendCustomVariablesToRequest(
          ...
          if (customVariablesStringified.length > 2) {
            return (
              ...
              parameterName +
              "=" +
              encodeWrapper(customVariablesStringified)
            );
          }
          return "";
        }
      ...
      function logEcommerce(
        ...
        if (ecommerceItems) {
          ...
          request +=
            "&ec_items=" + encodeWrapper(windowAlias.JSON.stringify(items));
        }
      ...
      this.setIgnoreCampaignsForReferrers = function (referrers) {
        configIgnoreCampaignsForReferrers = isString(referrers)
          ? [referrers]
          : referrers;
      };
