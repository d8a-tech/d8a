...
import ga4DuplicatorMin from '!!raw-loader!./ga4-duplicator/ga4-duplicator.min.js';
import ga4Duplicator from '!!raw-loader!./ga4-duplicator/ga4-duplicator.js';
...
This code duplicates all Google Analytics 4 tracking requests and redirects them to a custom endpoint while keeping the original Google Analytics functionality intact. It intercepts browser API calls (XMLHttpRequest, fetch, navigator.sendBeacon, and script src assignments) that websites commonly use to send tracking data. The below code is available in production-ready, minified form or for audit in the "Non-minified (for auditing)" tab or in the [source code](https://github.com/d8a-tech/d8a/tree/master/js/ga4-duplicator).
